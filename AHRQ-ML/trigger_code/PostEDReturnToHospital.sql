...
-- Set Variables
SET @SearchPeriodStart = '2019-01-01'
...
WHERE
	...
	AND
	prev.ReturnType LIKE 'EDToInpatient'
...
WHERE
	...
	(
		prev.ReturnType LIKE 'EDToInpatient'
		AND
		NOT DATEDIFF(DAY, prev.ReturnEncounterStartDateTime, prev.ReturnEncounterEndDateTime) = 0
	)
...
-- Print
IF (OBJECT_ID('Dflt._pk_RESULTS_AHRQML_PostEDReturnToHospital') IS NOT NULL)
	BEGIN
		DROP TABLE Dflt._pk_RESULTS_AHRQML_PostEDReturnToHospital
	END

SELECT * 
INTO Dflt._pk_RESULTS_AHRQML_PostEDReturnToHospital
FROM Dflt._pk_RESULTS_CONCLUSION

SELECT * FROM Dflt._pk_RESULTS_AHRQML_PostEDReturnToHospital
