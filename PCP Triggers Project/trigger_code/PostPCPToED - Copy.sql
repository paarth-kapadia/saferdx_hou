...
SET @SearchPeriodLength_Months = 1
...
SET @EDToInpatientLookforwardPeriodLength_Days = 14
...
--Variables
DECLARE @STEP02_v01_EDToInpatientStart INT
DECLARE @STEP02_v01_EDToInpatientLength INT
DECLARE @STEP02_v01_SearchStart DATETIME2
DECLARE @STEP02_v01_SearchEnd DATETIME2
DECLARE @STEP02_v01_Sta3n INT

SET @STEP02_v01_EDToInpatientStart = (SELECT params.EDToInpatientLookforwardPeriodStartDelta_Days FROM Dflt._pk_SETUP_ParameterTable AS params)
SET @STEP02_v01_EDToInpatientLength = (SELECT params.EDToInpatientLookforwardPeriodLength_Days FROM Dflt._pk_SETUP_ParameterTable AS params)
SET @STEP02_v01_SearchStart = (SELECT params.SearchPeriodStart FROM Dflt._pk_SETUP_ParameterTable AS params)
SET @STEP02_v01_SearchEnd = DATEADD(DAY, @STEP02_v01_EDToInpatientLength, DATEADD(MONTH, (SELECT params.SearchPeriodLength_Months FROM Dflt._pk_SETUP_ParameterTable AS params), (SELECT params.SearchPeriodStart FROM Dflt._pk_SETUP_ParameterTable AS params)))
SET @STEP02_v01_Sta3n = (SELECT params.StationID FROM Dflt._pk_SETUP_ParameterTable AS params)

PRINT CHAR(13) + CHAR(10) + 'Inclusion Step 02v01 - ED-to-Inpatient Period Start Delta (Days): ' + CAST(@STEP02_v01_EDToInpatientStart AS VARCHAR)
PRINT 'Inclusion Step 02v01 - ED-to-Inpatient Period Length (Days): ' + CAST(@STEP02_v01_EDToInpatientLength AS VARCHAR)
...
FROM
	Dflt._pk_INCLUSION_STEP02_v01 AS prev INNER JOIN Dflt._pk_INCLUSION_STEP01_vFIN AS idx_ed ON
	(
...
SELECT
	...
	,prev.ReturnEncounterStartDateTime
	,prev.ReturnEncounterEndDateTime
FROM Dflt._pk_INCLUSION_STEP02_v02 AS prev
...
SELECT DISTINCT prev.ReturnEncounterSID
FROM
	Dflt._pk_INCLUSION_CONCLUSION AS prev INNER JOIN Src.Inpat_InpatientDiagnosis AS inpd ON 
	(
		prev.ReturnEncounterSID = inpd.InpatientSID
	)
...
SELECT DISTINCT prev.ReturnEncounterSID
FROM
	Dflt._pk_INCLUSION_CONCLUSION AS prev INNER JOIN Src.Inpat_InpatientICDProcedure AS inpip ON 
	(
		prev.ReturnEncounterSID = inpip.InpatientSID
	)
...
SELECT DISTINCT prev.ReturnEncounterSID
FROM
	Dflt._pk_INCLUSION_CONCLUSION AS prev INNER JOIN Src.Outpat_WorkloadVDiagnosis AS owld ON 
	(
		prev.ReturnEncounterSID = owld.VisitSID
	)
		INNER JOIN CDWWork.Dim.ICD10 AS icd10 ON
		(
...
WHERE
	...
	(
		...
		FROM Dflt._pk_EXCLUSION_STEP01_v01 AS t
	)
...
CREATE TABLE Dflt._pk_EXCLUSION_STEP02_vFIN
(
	PatientSID BIGINT
	,IndexEncounterSID BIGINT
	,IndexEncounterStartDateTime DATETIME2
...
-- Get return encounters that are not inpatient encounters OR inpatient enocuntesr where the start and end date are on different days (representing an overnight stay)
--SHOULD THERE BE AN ED LENGTH CRITERIA??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
INSERT INTO Dflt._pk_EXCLUSION_STEP02_vFIN
SELECT
	prev.PatientSID
	,prev.IndexEncounterSID
	,prev.IndexEncounterStartDateTime
...
WHERE NOT DATEDIFF(DAY, prev.ReturnEncounterStartDateTime, prev.ReturnEncounterEndDateTime) = 0
...
CREATE TABLE Dflt._pk_EXCLUSION_STEP03_v01
(
	ReturnEncounterSID BIGINT
)


-- Get a list of the return encounters that are for external causes
INSERT INTO Dflt._pk_EXCLUSION_STEP03_v01
SELECT DISTINCT prev.ReturnEncounterSID
FROM
	Dflt._pk_EXCLUSION_STEP02_vFIN AS prev INNER JOIN Src.Inpat_InpatientDiagnosis AS inpd ON 
	(
		prev.ReturnEncounterSID = inpd.InpatientSID
	)
		INNER JOIN CDWWork.Dim.ICD10 AS icd10 ON
...
WHERE
	REPLACE(icd10.ICD10Code, '.', '') IN 
		(
			SELECT t.ICD10CMCode
			FROM Dflt._pk_SETUP_EncounterForAccident_ICD10CMCodes AS t
		)

INSERT INTO Dflt._pk_EXCLUSION_STEP03_v01
SELECT DISTINCT prev.ReturnEncounterSID
FROM
	Dflt._pk_EXCLUSION_STEP02_vFIN AS prev INNER JOIN Src.Outpat_WorkloadVDiagnosis AS owld ON 
	(
		prev.ReturnEncounterSID = owld.VisitSID
	)
		INNER JOIN CDWWork.Dim.ICD10 AS icd10 ON
...
CREATE TABLE Dflt._pk_EXCLUSION_STEP03_vFIN
(
	PatientSID BIGINT
	,IndexEncounterSID BIGINT
	,IndexEncounterStartDateTime DATETIME2
...
INSERT INTO Dflt._pk_EXCLUSION_STEP03_vFIN
SELECT 
	prev.PatientSID
	,prev.IndexEncounterSID
	,prev.IndexEncounterStartDateTime
	...
	,prev.ReturnEncounterStartDateTime
	,prev.ReturnEncounterEndDateTime
FROM Dflt._pk_EXCLUSION_STEP02_vFIN AS prev
WHERE
	prev.ReturnEncounterSID NOT IN
	(
		SELECT t.ReturnEncounterSID
		FROM Dflt._pk_EXCLUSION_STEP03_v01 AS t
	)
...
SELECT DISTINCT
	...
	,prev.IndexEncounterStartDateTime
	,prev.IndexEncounterEndDateTime
FROM Dflt._pk_EXCLUSION_STEP03_vFIN AS prev
...
CREATE TABLE Dflt._pk_EXCLUSION_STEP08_v01
(
	ReturnEncounterSID BIGINT
)
...
-- Get a list of the return encounters that are for external causes
INSERT INTO Dflt._pk_EXCLUSION_STEP08_v01
SELECT DISTINCT prev.ReturnEncounterSID
FROM
	Dflt._pk_EXCLUSION_STEP07_vFIN AS prev INNER JOIN Src.Inpat_InpatientDiagnosis AS inpd ON 
	(
		prev.ReturnEncounterSID = inpd.InpatientSID
	)
		INNER JOIN CDWWork.Dim.ICD10 AS icd10 ON
...
WHERE
	REPLACE(icd10.ICD10Code, '.', '') IN 
		(
			SELECT t.[ICD-10-CM Code]
			FROM Dflt._pk_UTIL_EncounterTypeICD_T2_Psych_T1_PsychAdmit AS t
		)

INSERT INTO Dflt._pk_EXCLUSION_STEP08_v01
SELECT DISTINCT prev.ReturnEncounterSID
FROM
	Dflt._pk_EXCLUSION_STEP07_vFIN AS prev INNER JOIN Src.Outpat_WorkloadVDiagnosis AS owld ON 
	(
		prev.ReturnEncounterSID = owld.VisitSID
	)
		INNER JOIN CDWWork.Dim.ICD10 AS icd10 ON
...
CREATE TABLE Dflt._pk_EXCLUSION_STEP08_vFIN
(
	PatientSID BIGINT
	,IndexEncounterSID BIGINT
	,IndexEncounterStartDateTime DATETIME2
...
-- Get patient records that don't have return encounters with external cause codes to exclude
INSERT INTO Dflt._pk_EXCLUSION_STEP08_vFIN
SELECT 
	prev.PatientSID
	,prev.IndexEncounterSID
	,prev.IndexEncounterStartDateTime
...
WHERE
	...
	(
		SELECT t.ReturnEncounterSID
		FROM Dflt._pk_EXCLUSION_STEP08_v01 AS t
	)
...
CREATE TABLE Dflt._pk_EXCLUSION_STEP09_v01
(
	ReturnEncounterSID BIGINT
)
...
-- Get a list of the return encounters that are for external causes
INSERT INTO Dflt._pk_EXCLUSION_STEP09_v01
SELECT DISTINCT prev.ReturnEncounterSID
FROM
	Dflt._pk_EXCLUSION_STEP08_vFIN AS prev INNER JOIN Src.Inpat_InpatientDiagnosis AS inpd ON 
	(
		prev.ReturnEncounterSID = inpd.InpatientSID
	)
		INNER JOIN CDWWork.Dim.ICD10 AS icd10 ON
...
WHERE
	REPLACE(icd10.ICD10Code, '.', '') IN 
		(
			SELECT t.[ICD-10-CM Code]
			FROM Dflt._pk_UTIL_EncounterTypeICD_T3_Accident_T1_ExternalCauses AS t
		)

INSERT INTO Dflt._pk_EXCLUSION_STEP09_v01
SELECT DISTINCT prev.ReturnEncounterSID
FROM
	Dflt._pk_EXCLUSION_STEP08_vFIN AS prev INNER JOIN Src.Outpat_WorkloadVDiagnosis AS owld ON 
	(
		prev.ReturnEncounterSID = owld.VisitSID
	)
		INNER JOIN CDWWork.Dim.ICD10 AS icd10 ON
...
CREATE TABLE Dflt._pk_EXCLUSION_STEP09_vFIN
(
	PatientSID BIGINT
	,IndexEncounterSID BIGINT
	,IndexEncounterStartDateTime DATETIME2
...
-- Get patient records that don't have return encounters with external cause codes to exclude
INSERT INTO Dflt._pk_EXCLUSION_STEP09_vFIN
SELECT 
	prev.PatientSID
	,prev.IndexEncounterSID
	,prev.IndexEncounterStartDateTime
	...
	,prev.ReturnEncounterStartDateTime
	,prev.ReturnEncounterEndDateTime
FROM Dflt._pk_EXCLUSION_STEP08_vFIN AS prev
WHERE
	prev.ReturnEncounterSID NOT IN
	(
		SELECT t.ReturnEncounterSID
		FROM Dflt._pk_EXCLUSION_STEP09_v01 AS t
	)
...
-- Create tables 
IF (OBJECT_ID('Dflt._pk_EXCLUSION_STEP10_v01') IS NOT NULL)
	BEGIN
		DROP TABLE Dflt._pk_EXCLUSION_STEP10_v01
	END

CREATE TABLE Dflt._pk_EXCLUSION_STEP10_v01
(
	ReturnEncounterSID BIGINT
)
...
-- Get a list of the return encounters that are for external causes
INSERT INTO Dflt._pk_EXCLUSION_STEP10_v01
SELECT DISTINCT prev.ReturnEncounterSID
FROM
	Dflt._pk_EXCLUSION_STEP09_vFIN AS prev INNER JOIN Src.Inpat_Inpatient AS inp ON 
	(
		prev.ReturnEncounterSID = inp.InpatientSID
	)
		INNER JOIN CDWWork.Dim.AdmitSource AS asrc ON
		(
			asrc.AdmitSourceSID = inp.AdmitSourceSID
		)
WHERE
	asrc.AdmitSourceCode NOT IN
	(
		'3B'
		,'2C'
		,'2B'
...
-- Create tables 
IF (OBJECT_ID('Dflt._pk_EXCLUSION_STEP10_vFIN') IS NOT NULL)
	BEGIN
		DROP TABLE Dflt._pk_EXCLUSION_STEP10_vFIN
	END

CREATE TABLE Dflt._pk_EXCLUSION_STEP10_vFIN
(
	PatientSID BIGINT
	,IndexEncounterSID BIGINT
	,IndexEncounterStartDateTime DATETIME2
...
-- Get patient records that don't have return encounters with external cause codes to exclude
INSERT INTO Dflt._pk_EXCLUSION_STEP10_vFIN
SELECT 
	prev.PatientSID
	,prev.IndexEncounterSID
	,prev.IndexEncounterStartDateTime
	...
	,prev.ReturnEncounterStartDateTime
	,prev.ReturnEncounterEndDateTime
FROM Dflt._pk_EXCLUSION_STEP09_vFIN AS prev
WHERE
	prev.ReturnEncounterSID NOT IN
	(
		SELECT t.ReturnEncounterSID
		FROM Dflt._pk_EXCLUSION_STEP10_v01 AS t
	)
...
SELECT
	...
	,prev.ReturnEncounterStartDateTime
	,prev.ReturnEncounterEndDateTime
FROM Dflt._pk_EXCLUSION_STEP10_vFIN AS prev
	...
	(
		...
		(
			...
			(
				...
				LEFT JOIN Src.Inpat_Inpatient AS r_inp ON
				(
					r_inp.InpatientSID = prev.ReturnEncounterSID
				)
					LEFT JOIN CDWWork.Dim.Sta3n AS r_sta ON
					(
						r_sta.Sta3n = r_inp.Sta3n
					)
...
SELECT COUNT(PatientSID) FROM Dflt._pk_EXCLUSION_STEP01_vFIN
SELECT COUNT(PatientSID) FROM Dflt._pk_EXCLUSION_STEP02_vFIN
SELECT COUNT(PatientSID) FROM Dflt._pk_EXCLUSION_STEP03_vFIN
...
SELECT COUNT(PatientSID) FROM Dflt._pk_EXCLUSION_STEP08_vFIN
SELECT COUNT(PatientSID) FROM Dflt._pk_EXCLUSION_STEP09_vFIN
SELECT COUNT(PatientSID) FROM Dflt._pk_EXCLUSION_STEP10_vFIN
SELECT COUNT(PatientSSN) FROM Dflt._pk_RESULTS_CONCLUSION
